...
(function($) {
	;$.extend({
		...
		window: function(params) {
			...
			var callback = (typeof params.callback === "function") ?  params.callback : false;
			//生成文档结构
			if(head){
			...
			$(".window-mask.no"+windowNum).on('click','.btn-cancel',function(){
				close()
			})
			$(".window-mask.no"+windowNum).on('click','.btn-ok',function(){
		...
		prompt:function(){
			...
			$(".window-mask-prompt").on("click",".btn-ok,.btn-cancel",function(e){
				if($(".btn-ok").is(e.target)){
					...
					if(inputValue === ""){
						$(".window-wrap-prompt").animate({"margin-top":"-10px"},62.5,"",function(){
							$(".window-wrap-prompt").animate({"margin-top":"0"},62.5)
						})
					}else{
